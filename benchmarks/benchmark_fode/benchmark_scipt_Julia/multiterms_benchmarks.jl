...
for n in range(3, 8)
    ...
    t6 = @benchmark solve($(prob), $(IMEX()), dt = $(h)); seconds=1
    ...
    sol6 =  solve(prob, IMEX(), dt = h);

    ery1=norm(sol1.u - analytical.(sol1.t), Inf)
    #ery2=sol2.errors[:final]
    ery3=norm(sol3.u - analytical.(sol3.t), Inf)
    ery4=norm(sol4.u - analytical.(sol4.t), Inf)
    ery5=norm(sol5.u - analytical.(sol5.t), Inf)
    ery6=norm(sol6.u - analytical.(sol6.t), Inf)
...
CSV.write("/Users/quqingyu/SciFracX/paper/benchmarks/data/FractionalDiffEq_IMEX.csv",  DataFrame(time = eval(Symbol("T6")), error = eval(Symbol("E6"))))
